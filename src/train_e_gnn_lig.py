...
class GraphListClassifier(nn.Module):
    ...
    def forward(self,inputs):
        for k,inp in enumerate(inputs):
            esm_embs,graph,ligs=inp
            res_emb=torch.cat([ligs,esm_embs.reshape(1,-1)],dim=1)
            # res_emb=esm_embs
            # res_emb=ligs
...
for fold in range(5):
    ...
    try:
        ...
        if INCLUDE_RES:
            ...
            if USE_ATTENTION:
                if USE_LIGAND_INFO:
                    model.load_state_dict(torch.load(f"{MODEL_PATH}/{metric}_models/attention_ligand_info_test/{model_name}_model_fold_{fold}.pth"))
                else:
