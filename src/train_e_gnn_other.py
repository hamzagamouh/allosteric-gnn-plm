...
print("GraphConv with dists as edge weights")
# Model Definition
class MLP(nn.Module):
...
class GCNWithReadout(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, readout="mean"):
        ...
        # K = 3
        # cutoff=12.0
        # interaction_cutoffs = torch.full((K,), cutoff)
        # # rbf_kernel_means = torch.linspace(0.5, 2.0, K)
        # # rbf_kernel_scaling = torch.ones(K) * 1.0
        # self.rbf_means = nn.Parameter(torch.linspace(0.5, cutoff, K))         # shape: (K,)
        # self.rbf_scaling = nn.Parameter(torch.ones(K)) 

        # # 5. Instantiate AtomicConv
        # self.layers.append(AtomicConv(interaction_cutoffs, self.rbf_means, self.rbf_scaling))
        # # self.layers.append(AtomicConv(interaction_cutoffs, rbf_kernel_means, rbf_kernel_scaling))
        # self.linear = nn.Linear(K * in_feats, hidden_feats)

        # EdgeGAT Conv
        # num_heads=1
        # self.layers.append(EGATConv(in_feats,1,hidden_feats,hidden_feats,num_heads=num_heads))
        # self.layers.append(EGATConv(hidden_feats,1,hidden_feats,hidden_feats,num_heads=num_heads))
    ...
    def forward(self, g):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Atomic Conv
            # h = layer(g, h, g.edata['dist'])
            # EdgeGAT Conv
            # h,attention_weights = layer(g, h, g.edata['dist'])
        ...
        #     h = layer(g, g.ndata['f'], g.edata['dist'])
        # h = self.linear(h)
        ...
        try:
            ...
            for i in idx:
                ...
                for g,lig_feats in zip(orig_graphs,pocket_ligands):
                    ...
                    # Get edge indices
                    # src, dst = g.edges()  # both are tensors of shape (E,)

                    # # Lookup source and destination coordinates
                    # src_pos = coords[src]  # shape: (E, 3)
                    # dst_pos = coords[dst]  # shape: (E, 3)
                    

                    # # Compute Euclidean distance per edge
                    # edge_dists = torch.norm(src_pos - dst_pos, dim=1, keepdim=True)  # shape: (E, 1)
                    # # Store in edge data
                    # g.edata['dist'] = edge_dists
                    g.ndata['lig']=lig_feat

                    # Lookup source and destination coordinates
                    # src_pos = coords  # shape: (E, 3)
                    # dst_pos = coords  # shape: (E, 3)
