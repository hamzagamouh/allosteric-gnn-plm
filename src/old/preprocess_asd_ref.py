...
def superimpose_residues(ref_residues,residues):
    ...
    for i,k in enumerate(superimposed_idx):
        final_ca_atoms[k]=ref_ca_atoms[i]
    return final_ca_atoms


def process_entry(unp,info):
    # if os.path.exists(f"{HOME_FOLDER}/allosteric/asd_processing/{unp}_ref_residues.pkl"):
    #     return
    try:
        ...
        # for entry in info['chains'][::-1]:
        for entry in [ref_entry]:
            # try:
        ...
        for entry,residues in all_residues.items():
            ...
            # final_residues[entry]=ref_residues
            # except Exception as e:
                # print(e)

        pickle.dump(final_residues,open(f"{HOME_FOLDER}/allosteric/asd_processing/{unp}_ref_residues.pkl",'wb'))        
    except KeyboardInterrupt:
        sys.exit(0)
...
if __name__=="__main__":
    ...
    #     process_entry(unp,info)
    #     break
    Parallel(n_jobs=-1, backend='multiprocessing', verbose=1)(
        delayed(process_entry)(unp,info) for unp,info in asd_entries.items()
    )
