...
import dgl.readout as dgl_readout


from e_gnn_utils import glorot_orthogonal, calculate_and_store_dists_in_graph, src_dot_dst, scaling, \
...
class GraphRep(nn.Module):
    ...
    def message_func(self, edges: dgl.udf.EdgeBatch):
        ...
        """
        edge_mlp_input = torch.cat([edges.src['f'], edges.dst['f'], edges.data['r']], dim=1)
    ...
    def forward(
        ...
        # pooled_out=graph.ndata['f'].sum(0)
        return dgl_readout.sum_nodes(graph, 'f')
